...
export const Organization = () => {
  return (
    <div className=' max-h-2/5 h-80 flex justify-center items-center py-2'>
      <section className='flex flex-col justify-center items-center  gap-12'>
      <h1 className='text-2xl font-bold tracking-wide'>Trusted by 2000+ Organization</h1>
      <div className='flex justify-center items-center space-x-20 px-2'>
        {image.map((item) => (
          <Image key={item.id} src={item.image} alt={item.id.toString()} className='size-fit object-cover'/>
        ))}
      </div>
      </section>
    </div>
